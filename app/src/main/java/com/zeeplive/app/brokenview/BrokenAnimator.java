...
class BrokenAnimator extends ValueAnimator{
    ...
    private BrokenViews mBrokenViews;
    private View mView;
    ...
    public BrokenAnimator(BrokenViews brokenViews, View view, Bitmap bitmap, Point point, BrokenConfig config){
        mBrokenViews = brokenViews;
        mView = view;
        mBitmap = bitmap;
        mTouchPoint = point;
        mConfig = config;
        ...
        mBrokenViews.getGlobalVisibleRect(brokenViewR);
    ...
    private void buildBrokenLines(Rect r) {
        ...
        for (int i = 0; i < mConfig.complexity; i++) {
            ...
            if (length > Util.dp2px(THRESHOLD)) {
    ...
    private void buildBaselines(LinePath[] baseLines, Rect r){
        ...
        for(int i = 1; i<mConfig.complexity; i++) {
            ...
            angleRandom = angle +Util.nextInt(-range, range);
            if (angleRandom >= 360)
                angleRandom -= 360;
    ...
    private void buildFirstLine(LinePath path, Rect r){
        ...
        switch (maxId){
            case 0:
                path.setEndPoint(r.left, Util.nextInt(r.height()) + r.top);
                break;
            case 1:
                path.setEndPoint(Util.nextInt(r.width()) + r.left, r.top);
                break;
            case 2:
                path.setEndPoint(r.right, Util.nextInt(r.height()) + r.top);
                break;
            case 3:
                path.setEndPoint(Util.nextInt(r.width()) + r.left, r.bottom);
                break;
        }
        path.lineToEnd();
    }
    ...
    private void buildBrokenAreas(Rect r){
        ...
        for(int i = 0; i < mConfig.complexity; i++) {
            ...
            lineRifts[i].setStartLength(Util.dp2px(START_LENGTH));

            if (repeat > 0) {
                repeat--;
            } else {
                linkLen = Util.nextInt(Util.dp2px(SEGMENT_LESS), Util.dp2px(SEGMENT));
                repeat = Util.nextInt(3);
            }
            ...
            if (hasCircleRifts && pmNow.getLength() > linkLen && pmPre.getLength() > linkLen) {
                ...
                circleWidth[i] = Util.nextInt(Util.dp2px(1)) + 1;
    ...
    private void buildPieces(){
        ...
        for(int i = 0; i < pieces.length; i++) {
            int shadow = Util.nextInt(Util.dp2px(2), Util.dp2px(9));
            Path path = pathArray.get(i);
            RectF r = new RectF();
            path.computeBounds(r, true);

            Bitmap pBitmap = Util.createBitmapSafely((int)r.width() + shadow * 2,
    ...
    private void warpStraightLines() {
        ...
        for (int i = 0; i < mConfig.complexity; i++) {
            if(lineRifts[i].isStraight())
            {
                ...
                int xRandom = (int) (pos[0] + Util.nextInt(-Util.dp2px(1),Util.dp2px(1)));
                int yRandom = (int) (pos[1] + Util.nextInt(-Util.dp2px(1), Util.dp2px(1)));
                lineRifts[i].reset();
                lineRifts[i].moveTo(0,0);
                lineRifts[i].lineTo(xRandom,yRandom);
                lineRifts[i].lineToEnd();
    ...
    public boolean draw(Canvas canvas) {
        ...
        if(getStage() == STAGE_BREAKING) {
            ...
            if(piecesNum == 0) {
                ...
                mBrokenViews.onBrokenFallingEnd(mView);
            }
        }
